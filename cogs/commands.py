...
class Tools(discord.ext.commands.Cog): #Admin tools stuffs
    language_group = discord.app_commands.Group(name = "language", description = "Commands related to the language the bot is using")
    ...
    async def off(self, inter: discord.Interaction): #Turn off comand
        file = open(f"../lang/{self.SERVER_SETTINGS.find_one({'_id': inter.guild_id})['language']}.json") #Get server language
        await inter.response.send_message(json.load(file)["tools"]["off"]["message"]) #Send correct message
        file.close()

        cogs.rakbotbase.Functions().write_log("Turning off") #Send logs

        self.RAKBOT.tree.clear_commands(guild = None)
    ...
    @off.error
    async def off_error(self, inter: discord.Interaction): #If off failed
        file = open(f"../lang/{self.SERVER_SETTINGS.find_one({'_id': inter.guild_id})['language']}.json") #Get server language
        await inter.response.send_message(json.load(file)["tools"]["off"]["error"]) #Send correct message
        file.close()

        cogs.rakbotbase.Functions().write_log(f"{inter.user.display_name} ({inter.user}) got angry and tried to turn me off")

    @language_group.command(name = "get", description = "Check server language")
    async def language_top(self, inter: discord.Interaction): #Check the language of the server
        file = open(f"../lang/{self.SERVER_SETTINGS.find_one({'_id': inter.guild_id})['language']}.json") #Get server language
        await inter.response.send_message(json.load(file)["tools"]["language"]["message"]) #Send correct message
        file.close()

        cogs.rakbotbase.Functions().write_log(f"{inter.user.display_name} ({inter.user}) checked the language of the {inter.guild.name} server")

    @language_group.command(name = "list", description = "List all the available languages")
    @discord.ext.commands.has_permissions(administrator = True)
    async def language_list(self, inter: discord.Interaction): #Check all the available languages
        e = discord.Embed(colour = 0xFF6D00) #Declare embed

        file = open(f"../lang/{self.SERVER_SETTINGS.find_one({'_id': inter.guild_id})['language']}.json") #Get server language
        e.set_author(name = json.load(file)["tools"]["language"]["list"]["message"], icon_url = self.RAKBOT.user.avatar)
        file.close()
        ...
        await inter.response.send_message(embed = e) #Send it
        cogs.rakbotbase.Functions().write_log(f"{inter.user.display_name} ({inter.user}) asked for list of languages")

    @discord.ext.commands.has_permissions(administrator = True)
    @language_group.command(name = "set", description = "Choose the language for the server")
    async def language_set(self, inter: discord.Interaction, language: str): #Set the language for the current server
        if f"{language}.json" in os.listdir("../lang"): #If the language exists
            self.SERVER_SETTINGS.update_one({"_id": inter.guild_id}, {"$set": {"language": language}}) #Change the value in the database

            file = open(f"../lang/{language}.json")
            await inter.response.send_message(json.load(file)["tools"]["language"]["set"]["message"])
            file.close()

            cogs.rakbotbase.Functions().write_log(f"{inter.user.display_name} ({inter.user}) set a new language for the server {inter.guild.name}")
        else:
            file = open(f"../lang/{self.SERVER_SETTINGS.find_one({'_id': inter.guild_id})['language']}.json") #Get server language
            await inter.response.send_message(json.load(file)["tools"]["language"]["set"]["error"])
            file.close()

            cogs.rakbotbase.Functions().write_log(f"{inter.user.display_name} ({inter.user}) failed to set a new language for the server {inter.guild.name}")

    @discord.app_commands.command(name = "shout", description = "Wake someone up")
    async def shout(self, inter: discord.Interaction, member: discord.Member): #Command to spam a user to wake them up
        ...
        await inter.response.send_message(message_parts[0]+member.mention+message_parts[1]) #Send correct message
        file.close()

        cogs.rakbotbase.Functions().write_log(f"{inter.user.display_name} ({inter.user}) shouts at {member.display_name} ({member})")
